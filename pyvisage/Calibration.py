...
class Calibration(object):
    ...
    def mA2Lux(self, LEDintensity):
        ...
        if LEDintensity < self.LED_intensity[0]: raise("LED intensity provided (%f) is smaller than the smallest value (%f) available in the calibration file (%s)"%(LEDintensity, self.LED_intensity[0],self.file_calibration))
        if LEDintensity > self.LED_intensity[-1]: raise("LED intensity provided (%f) is larger than the largest value (%f) available in the calibration file (%s)"%(LEDintensity, self.LED_intensity[-1],self.file_calibration))
