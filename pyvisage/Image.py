...
class Image(object):
    ...
    def column_swap(self):
        ...
        arr_swap = numpy.insert(arr_swap_col0,range(0,arr_swap_col1.shape[1]),arr_swap_col1,axis=1)

        # merge odd and even rows back
        self.array = numpy.insert(arr_fix,range(1,arr_swap.shape[0]+1),arr_swap,axis=0)
    ...
    def calculate_RTN_correction(self, bswap=False, colstart=4, colstop=67, target=None, boddeven=False, bsave=False, filename=None):
        ...
        for i_adc in range(number_adc):
            ...
            if target==None and boddeven:
                ...
                rtn_corr_oddeven = numpy.insert(rtn_corr_even,range(1,rtn_corr_odd.shape[0]+1),rtn_corr_odd)
        ...
        if bsave:
            ...
            for i_adc in range(number_adc):
                ...
                pylab.plot(range(self.rows),[v*correc_fact for v in rtn_correction_factors[i_adc]])
            title += ' DN'
            pylab.xlabel('Rows')
    ...
    def apply_RTN_correction(self,rtn_correction_factors,bswap=False):
        ...
        array_RTNcorrected = numpy.insert(array_RTNcorrected,range(1,arr_adc[1].shape[1]+1),arr_adc[1],axis=1)
        array_RTNcorrected = numpy.insert(array_RTNcorrected,range(2,arr_adc[2].shape[1]*2+1,2),arr_adc[2],axis=1)
        array_RTNcorrected = numpy.insert(array_RTNcorrected,range(3,arr_adc[3].shape[1]*3+1,3),arr_adc[3],axis=1)
    ...
    def correlate(self,image,channel=None,bPearson=True,bplot=True,figsize=9):
        ...
        if channel==None:
            channels_to_plot = range(self.num_channel_groups)
        else:
        ...
        for i_plot,i_channel_group in enumerate(channels_to_plot):
            ...
            if bplot:
                ...
                if bPearson:
                    ...
                    delta = range(-(len(correlation_array)-1)/2,(len(correlation_array)-1)/2+1)
    ...
    def plot_histogram(self, show_channels=None,bsigma=False,xmin=None, xmax=None, nbins=100, ylim=None, blog=False, figsize=9, bsave=False, filename="fig_hist.png", title='', xlabel="DN", ylabel="# pixels", color='b', legend_label=None, legend_location='upper center', bfit=False, image2=None, color2='r',legend_label2=None):
        ...
        if show_channels==None:
            show_channels = range(self.num_channel_groups)
            arr = self.get_array()
    ...
    def plot_profile(self, axis=0, bstack=False, show_channels=None, figsize=9, xlim=None, ylim=None, bsave=False, filename="fig_profile.png", title=None, xlabel=None, ylabel=None, color='b', lw=2, legend_label=None, legend_location='upper center', image2=None, color2='r', legend_label2=None):
        ...
        for i_plot,i_channel_group in enumerate(show_channels):
            ...
            if legend_label==None:
                pylab.plot(range(len(profile_array)),profile_array,color=color,linewidth=lw)
            else:
                pylab.plot(range(len(profile_array)),profile_array,color=color,linewidth=lw,label=legend_label)
            ...
            if image2 != None:
                ...
                if legend_label2==None:
                    pylab.plot(range(len(profile_array2)),profile_array2,color=color2,linewidth=lw)
                else:
                    pylab.plot(range(len(profile_array2)),profile_array2,color=color2,linewidth=lw,label=legend_label2)
